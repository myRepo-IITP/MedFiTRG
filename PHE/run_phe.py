#!/usr/bin/env python3
"""
Main script for training and testing the Phenotype multimodal model.
Usage: python run_phe.py [--mode train|test|both] [--gpu GPU_ID] [--epochs NUM_EPOCHS] [--batch_size BATCH_SIZE]
"""
...
from evaluate import test_model, print_results
from losses import RobustFocalLoss
from utils import calculate_class_weights

def parse_args():
    parser = argparse.ArgumentParser(description="Train/Test Phenotype Multimodal Model")
...
def get_phenotype_columns(config):
    """Extract phenotype columns from metadata"""
    sample_df = pd.read_csv(config.TRAIN_META)
    phenotype_columns = sample_df.columns[6:].tolist()  # Skip first 6 columns
    print(f"[DATA] Found {len(phenotype_columns)} phenotype columns")
...
def main():
    ...
    print(f"[TASK] Phenotype Prediction - Multi-label Classification")
    ...
    phenotype_columns = get_phenotype_columns(config)
    num_classes = len(phenotype_columns)
    print(f"[MODEL] Number of output classes (phenotypes): {num_classes}")
    
    # Setup dataloaders
    train_loader, val_loader, test_loader, train_ds = setup_dataloaders(
        config, tokenizer, phenotype_columns
    )
    
    # Setup models
    ehr_model, cxr_model, text_model, fusion_model = setup_models(
        config, train_ds, num_classes, device
    )
    ...
    print(f"  Classifier: {num_classes} outputs for multi-label phenotype prediction")
    
    if args.mode in ['train', 'both']:
        # Calculate class weights
        pos_weights = calculate_class_weights(train_loader, num_classes)
        pos_weights = pos_weights.to(device)
        
        # Setup loss function
        criterion = RobustFocalLoss(
    ...
    if args.mode in ['test', 'both']:
        ...
        print(f"Phenotype Prediction Performance Summary:")
        ...
        print(f"  Multi-label Classification: {num_classes} phenotypes")
