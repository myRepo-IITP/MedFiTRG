...
class PatientGraphDataset(Dataset):
    def __init__(self, metadata_csv, ehr_base_dir, cxr_base_dir, split_name, phenotype_columns, tokenizer, max_len=512):
        ...
        # Sort so we can determine visit order correctly
    ...
    def __getitem__(self, idx):
        ...
        for visit_order, (visit_idx, row) in enumerate(patient_visits.iterrows()):
            ...
            # Phenotype labels (multi-label)
            labels = torch.tensor(row[self.phenotype_columns].values.astype(np.float32))
