...
from dataset import REAGraphDataset, REAGraphCollator
from losses import RobustFocalLoss
from utils import calculate_class_weights, train_epoch_rea, evaluate_rea

def setup_dataloaders(config, tokenizer):
    """Setup train, validation, and test dataloaders for readmission prediction"""
    print("[DATA] Loading readmission datasets...")
    
    # Datasets for readmission prediction
    train_ds = REAGraphDataset(config.TRAIN_META, config.EHR_BASE_DIR, config.CXR_BASE_DIR, 
                               "train", tokenizer)
    val_ds = REAGraphDataset(config.VAL_META, config.EHR_BASE_DIR, config.CXR_BASE_DIR, 
                             "val", tokenizer)
    test_ds = REAGraphDataset(config.TEST_META, config.EHR_BASE_DIR, config.CXR_BASE_DIR, 
                              "test", tokenizer)
    
    print(f"  Training patient stays: {len(train_ds)}")
    print(f"  Validation patient stays: {len(val_ds)}")
    print(f"  Test patient stays: {len(test_ds)}")
    
    collator = REAGraphCollator(max_len=config.MAX_TEXT_LENGTH)
...
def setup_models(config, train_ds, device):
    """Initialize all models for readmission prediction"""
    # Feature Extractors
    ...
    # Fusion Model for readmission (binary classification)
    fusion_model = TripleFiLMNetwork(
        ...
        hidden_dim=config.HIDDEN_DIM, num_classes=config.NUM_CLASSES, film_hidden_dim=config.FILM_HIDDEN_DIM, 
...
def train_model(config, train_loader, val_loader, device, 
    ...
    """Main training loop for readmission prediction"""
    best_auroc = 0.0
    best_threshold = 0.5
    epochs_no_imp = 0
    ...
    for epoch in range(1, config.NUM_EPOCHS + 1):
        train_loss = train_epoch_rea(fusion_model, train_loader, optimizer, criterion, 
                                     ehr_model, cxr_model, text_model, device, config)
        val_metrics, val_threshold = evaluate_rea(fusion_model, val_loader, criterion, 
        ...
        print(f"{epoch:<6} | {train_loss:<9.4f} | {val_metrics['loss']:<9.4f} | "
              f"{val_metrics['auroc']:<8.4f} | {val_metrics['f1_score']:<8.4f} | "
              f"{val_metrics['auprc']:<8.4f} | {val_threshold:<7.3f} | {current_lr:<10.2e}")
